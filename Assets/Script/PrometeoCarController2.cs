...
public class PrometeoCarController2 : MonoBehaviour
{
    //CAR SETUP
    ...
    */
    ...
    void Update()
    {
        ...
        */
        ...
        {
            if ( /*Input.GetKey(KeyCode.W)*/ _inputs.Roll > 0.1f)
            {
                CancelInvoke("DecelerateCar");
                deceleratingCar = false;
                GoForward();
            ...
            if ( /*Input.GetKey(KeyCode.S)*/ _inputs.Roll < 0f)
            {
                CancelInvoke("DecelerateCar");
                deceleratingCar = false;
                GoReverse();
            ...
            if ( /*Input.GetKey(KeyCode.A)*/ _inputs.Turn < 0f)
            {
                TurnLeft();
            }

            if ( /*Input.GetKey(KeyCode.D)*/  _inputs.Turn > 0.1f)
            {
                TurnRight();
            }

            // if ( /*Input.GetKey(KeyCode.Space)*/ _inputs.Handbrake)
            // {
            //     CancelInvoke("DecelerateCar");
            //     deceleratingCar = false;
            //     Handbrake();
            ...
            // if (/*Input.GetKeyUp(KeyCode.Space*/ _inputs.Handbrake)
            // {
            //     GetKeyUp = true;
            // }
            ...
            if ((!(_inputs.Roll < 0f) /*Input.GetKey(KeyCode.S)*/ &&
                 !(_inputs.Roll > 0.1f) /*Input.GetKey(KeyCode.W*/))
            {
                ThrottleOff();
            }

            if (( /*Input.GetKey(KeyCode.S)*/
                    !(_inputs.Roll < 0f) && /*Input.GetKey(KeyCode.W)*/!(_inputs.Roll > 0.1f)) /*Input.GetKey(KeyCode.Space)*/ &&
                !deceleratingCar)
            {
                InvokeRepeating("DecelerateCar", 0f, 0.1f);
                deceleratingCar = true;
            ...
            if (!(_inputs.Turn < 0f) && !(_inputs.Turn > 0.1f) && steeringAxis != 0f)
            {
                ResetSteeringAngle();
            }
        }
